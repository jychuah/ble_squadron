...
#define     FRONT_REAR_ARC      2
#define     LEFT_RIGHT_ARC      3
...
#define     RED_DUTY_CYCLE      32000
#define     WHITE_DUTY_CYCLE    65000
...
class Lights {
  public:
    ...
    int engine_pulse = 40;
    int engine_rate = 800;
    int headlight_intensity = 90;
    int arc_intensity = 50;
    int current_firing_arc = ARC_LAT;

    int engine_pins [ NUM_ENGINES ] = { 7, 8, 10, 11 };
    int engine_values[ NUM_ENGINES ] = { 0, 0, 0, 0 };
    int engine_limits [ NUM_ENGINES ] = { RED_DUTY_CYCLE, RED_DUTY_CYCLE, RED_DUTY_CYCLE, RED_DUTY_CYCLE };
    ...
    void set_limit(int pin) {
        ...
        engine_limits[pin] = engine_directions[pin] * random(percent / 2, percent) / 100.0 * RED_DUTY_CYCLE + engine_values[pin];
        if (engine_limits[pin] > RED_DUTY_CYCLE) {
          engine_limits[pin] = RED_DUTY_CYCLE;
        }
    ...
    void set_runninglights(int val) {
      setPin(RUNNING_LIGHTS, val);
    }

    void set_long_arcs(int val) {
      setPin(FRONT_REAR_ARC, val);
    }

    void set_lateral_arcs(int val) {
      setPin(LEFT_RIGHT_ARC, val);
    }
    ...
    Lights() {
      ...
      for (int i = 0; i < NUM_ENGINES; i++) {
        ...
        engine_values[i] = engine_glow / 100.0 * RED_DUTY_CYCLE;        
        set_limit(i);
      }
    ...
    void pulse_engines() {
      set_runninglights( (int)(WHITE_DUTY_CYCLE * headlight_intensity / 100.0));
      ...
      if (!effectPlaying) {
        ...
        for (int i = 0; i < NUM_ENGINES ; i++) {
          ...
          if (engine_values[i] > RED_DUTY_CYCLE) {
            engine_values[i] = RED_DUTY_CYCLE;
          }
        ...
        if (current_firing_arc == ARC_LAT) {
          set_lateral_arcs( (int)(RED_DUTY_CYCLE * arc_intensity / 100.0));
          set_long_arcs( 0 );
        } else {
          set_lateral_arcs( 0 );
          set_long_arcs( (int)(RED_DUTY_CYCLE * arc_intensity / 100.0));
        }
      }
//      ledDriver.write();
    ...
    void blaster() {
      for (int i = 0; i < 14; i++) {
        for (int j = 1; j < RED_DUTY_CYCLE; j = j * 4) {
          set_lateral_arcs(j);
          set_long_arcs(j);
          delay(18);
        }
    ...
    void hyperdrive() {
      ...
      set_engines(RED_DUTY_CYCLE);
      delay(500);
      for (int i = RED_DUTY_CYCLE; i > 15000; i = i - 64) {
        set_engines(i);
        delay(1);
      }
    }
    ...
    void launch() {
      ...
      set_runninglights(0);
      delay(250);
      set_runninglights(10000);
      delay(250);
      set_runninglights(0);
      delay(250);
      set_runninglights(10000);
      delay(250);
      set_runninglights(32000);
      for (int i = 100; i < 15000; i = i + 7) {
        set_engines(i);
        delay(1);
      }
      for (int i = 0; i < 10; i++) {
        for (int j = RED_DUTY_CYCLE / 2; j < RED_DUTY_CYCLE; j = j + 300) {
          set_engines(j);
          delay(1);
        }
        for (int j = RED_DUTY_CYCLE; j > RED_DUTY_CYCLE / 2; j = j - 300) {
          set_engines(j);
          delay(1);
        }
      }
    ...
    void notmyfault() {
      set_engines(300);
      delay(1000);
      for (int i = 1000; i < RED_DUTY_CYCLE; i = i + 350) {
        set_engines(i);
        delay(10);
      }
      set_engines(RED_DUTY_CYCLE);
      for (int i = 0; i < 10; i++) {
        for(int j = RED_DUTY_CYCLE - i * 5000; j > 1; j = j / 2) {
          set_engines(j);
          delay(25 + i);
        }
      }
...
void setup() {
  Serial.begin(115200);
  Serial.println("VT-49 Decimator");
